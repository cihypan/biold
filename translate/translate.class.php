...
function validateLang($langPair) {
  ...
  //echo $langPair; 
  //print_r ($this->validTranslations);
...
//   $search_regex='/<textarea name=q rows=5 cols=45 wrap=PHYSICAL dir=ltr>(.*)<\/textarea>\&nbsp;\&nbsp;<input type=hidden/';
	$search_regex='/<div id=result_box dir=ltr>(.*)<\/div><\/td><\/tr>/';
...
function buildQuery() {
   ...
   if (($this->langTo || $this->langFrom || $this-> text) == ("" || NULL))
     return("Please set language to, language from, and the text to be translated");
